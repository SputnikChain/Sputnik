...
CzSputnikTracker::CzSputnikTracker(std::string strWalletFile)
{
...
CzSputnikTracker::~CzSputnikTracker()
{
    mapSerialHashes.clear();
    mapPendingSpends.clear();
}

void CzSputnikTracker::Init()
{
...
bool CzSputnikTracker::Archive(CMintMeta& meta)
{
...
bool CzSputnikTracker::UnArchive(const uint256& hashPubcoin, bool isDeterministic)
{
    CWalletDB walletdb(strWalletFile);
    if (isDeterministic) {
        CDeterministicMint dMint;
...
CMintMeta CzSputnikTracker::Get(const uint256 &hashSerial)
{
...
CMintMeta CzSputnikTracker::GetMetaFromPubcoin(const uint256& hashPubcoin)
{
...
bool CzSputnikTracker::GetMetaFromStakeHash(const uint256& hashStake, CMintMeta& meta) const
{
    for (auto& it : mapSerialHashes) {
...
std::vector<uint256> CzSputnikTracker::GetSerialHashes()
{
...
CAmount CzSputnikTracker::GetBalance(bool fConfirmedOnly, bool fUnconfirmedOnly) const
{
    CAmount nTotal = 0;
...
CAmount CzSputnikTracker::GetUnconfirmedBalance() const
{
    return GetBalance(false, true);
}

std::vector<CMintMeta> CzSputnikTracker::GetMints(bool fConfirmedOnly) const
{
...
bool CzSputnikTracker::HasMintTx(const uint256& txid)
{
    for (auto it : mapSerialHashes) {
...
bool CzSputnikTracker::HasPubcoin(const CBigNum &bnValue) const
{
...
bool CzSputnikTracker::HasPubcoinHash(const uint256& hashPubcoin) const
{
    for (auto it : mapSerialHashes) {
        CMintMeta meta = it.second;
        if (meta.hashPubcoin == hashPubcoin)
...
bool CzSputnikTracker::HasSerial(const CBigNum& bnSerial) const
{
    uint256 hash = GetSerialHash(bnSerial);
    return HasSerialHash(hash);
}

bool CzSputnikTracker::HasSerialHash(const uint256& hashSerial) const
{
    auto it = mapSerialHashes.find(hashSerial);
    return it != mapSerialHashes.end();
}

bool CzSputnikTracker::UpdateZerocoinMint(const CZerocoinMint& mint)
{
...
bool CzSputnikTracker::UpdateState(const CMintMeta& meta)
{
    CWalletDB walletdb(strWalletFile);
...
void CzSputnikTracker::Add(const CDeterministicMint& dMint, bool isNew, bool isArchived)
{
    CMintMeta meta;
...
void CzSputnikTracker::Add(const CZerocoinMint& mint, bool isNew, bool isArchived)
{
    CMintMeta meta;
...
void CzSputnikTracker::SetPubcoinUsed(const uint256& hashPubcoin, const uint256& txid)
{
...
void CzSputnikTracker::SetPubcoinNotUsed(const uint256& hashPubcoin)
{
    if (!HasPubcoinHash(hashPubcoin))
        return;
    CMintMeta meta = GetMetaFromPubcoin(hashPubcoin);
...
void CzSputnikTracker::RemovePending(const uint256& txid)
{
    uint256 hashSerial;
...
bool CzSputnikTracker::UpdateStatusInternal(const std::set<uint256>& setMempool, CMintMeta& mint)
{
...
std::set<CMintMeta> CzSputnikTracker::ListMints(bool fUnusedOnly, bool fMatureOnly, bool fUpdateStatus)
{
    CWalletDB walletdb(strWalletFile);
    if (fUpdateStatus) {
        ...
        for (auto& dMint : listDeterministicDB)
            Add(dMint);
        LogPrint("zero", "%s: added %d dzSputnik from DB\n", __func__, listDeterministicDB.size());
    }
