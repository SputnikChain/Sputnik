...
WalletModel::SendCoinsReturn WalletModel::prepareTransaction(WalletModelTransaction& transaction, const CCoinControl* coinControl)
{
    ...
    foreach (const SendCoinsRecipient& rcp, recipients) {
        if (rcp.paymentRequest.IsInitialized()) { // PaymentRequest...
            ...
            total += subtotal;
        } else { // User-entered Sputnik address / amount:
...

    {
        ...
        if (recipients[0].useSwiftTX && total > GetSporkValue(SPORK_5_MAX_VALUE) * COIN) {
            emit message(tr("Send Coins"), tr("SwiftX doesn't support sending values that high yet. Transactions are currently limited to %1 Sputnik.").arg(GetSporkValue(SPORK_5_MAX_VALUE)),
...
WalletModel::SendCoinsReturn WalletModel::sendCoins(WalletModelTransaction& transaction)
{
...

    {
        ...
        foreach (const SendCoinsRecipient& rcp, recipients) {
            ...
            } else if (!rcp.message.isEmpty()) // Message from normal Sputnik:URI (Sputnik:XyZ...?message=example)
            {
...
static void NotifyzSputnikReset(WalletModel* walletmodel)
{
    qDebug() << "NotifyzSputnikReset";
...
void WalletModel::subscribeToCoreSignals()
{
    ...
    wallet->NotifyzSputnikReset.connect(boost::bind(NotifyzSputnikReset, this));
...
void WalletModel::unsubscribeFromCoreSignals()
{
    ...
    wallet->NotifyzSputnikReset.disconnect(boost::bind(NotifyzSputnikReset, this));
...
{
    setMints.clear();
    setMints = pwalletMain->zSputnikTracker->ListMints(fUnusedOnly, fMaturedOnly, fUpdateStatus);
}
