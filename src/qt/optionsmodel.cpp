...
void OptionsModel::Init()
{
    ...
    if (!settings.contains("nDisplayUnit"))
        settings.setValue("nDisplayUnit", BitcoinUnits::Sputnik);
    ...
    if (!settings.contains("nAnonymizeSputnikAmount"))
        settings.setValue("nAnonymizeSputnikAmount", 1000);

    nAnonymizeSputnikAmount = settings.value("nAnonymizeSputnikAmount").toLongLong();
...
// Wallet
    ...
    if (settings.contains("nAnonymizeSputnikAmount"))
        SoftSetArg("-anonymizeSputnikamount", settings.value("nAnonymizeSputnikAmount").toString().toStdString());
...
void OptionsModel::Reset()
{
    ...
    settings.clear();
    resetSettings = true; // Needed in Sputnik.cpp during shotdown to also remove the window positions
...
QVariant OptionsModel::data(const QModelIndex& index, int role) const
{
    if (role == Qt::EditRole) {
        ...
        case AnonymizeSputnikAmount:
            return QVariant(nAnonymizeSputnikAmount);
        case Listen:
...
bool OptionsModel::setData(const QModelIndex& index, const QVariant& value, int role)
{
    ...
    if (role == Qt::EditRole) {
        ...
        case AnonymizeSputnikAmount:
            nAnonymizeSputnikAmount = value.toInt();
            settings.setValue("nAnonymizeSputnikAmount", nAnonymizeSputnikAmount);
            emit anonymizeSputnikAmountChanged(nAnonymizeSputnikAmount);
            break;
        case CoinControlFeatures:
