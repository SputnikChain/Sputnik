...
// Copyright (c) 2017-2018 The Sputnik developers
// Distributed under the MIT/X11 software license, see the accompanying
// file COPYING or http://www.opensource.org/licenses/mit-license.php.

#include "zSputnikcontroldialog.h"
#include "ui_zSputnikcontroldialog.h"
...
std::set<std::string> ZSputnikControlDialog::setSelectedMints;
std::set<CMintMeta> ZSputnikControlDialog::setMints;

ZSputnikControlDialog::ZSputnikControlDialog(QWidget *parent) :
    QDialog(parent, Qt::WindowSystemMenuHint | Qt::WindowTitleHint | Qt::WindowCloseButtonHint),
    ui(new Ui::ZSputnikControlDialog),
    model(0)
{
    ui->setupUi(this);
    setMints.clear();
...
ZSputnikControlDialog::~ZSputnikControlDialog()
{
    delete ui;
}

void ZSputnikControlDialog::setModel(WalletModel *model)
{
    this->model = model;
    updateList();
}
...
void ZSputnikControlDialog::updateList()
{
...
void ZSputnikControlDialog::updateSelection(QTreeWidgetItem* item, int column)
{
...
void ZSputnikControlDialog::updateLabels()
{
    int64_t nAmount = 0;
    for (const CMintMeta& mint : setMints) {
    ...
    ui->labelZSputnik_int->setText(QString::number(nAmount));
    ui->labelQuantity_int->setText(QString::number(setSelectedMints.size()));

}

std::vector<CMintMeta> ZSputnikControlDialog::GetSelectedMints()
{
...
void ZSputnikControlDialog::ButtonAllClicked()
{
    ui->treeWidget->blockSignals(true);
