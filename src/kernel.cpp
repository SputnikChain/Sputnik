...
bool CheckProofOfStake(const CBlock block, uint256& hashProofOfStake, std::unique_ptr<CStakeInput>& stake)
{
    ...
    if (tx.IsZerocoinSpend()) {
        ...
        stake = std::unique_ptr<CStakeInput>(new CZSputnikStake(spend));
    } else {
        ...
        CSputnikStake* SputnikInput = new CSputnikStake();
        SputnikInput->SetInput(txPrev, txin.prevout.n);
        stake = std::unique_ptr<CStakeInput>(SputnikInput);
    }
