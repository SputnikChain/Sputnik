...
CzSputnikWallet::CzSputnikWallet(std::string strWalletFile)
{
    ...
    //Check for old db version of storing zSputnik seed
    if (fFirstRun) {
        uint256 seed;
        if (walletdb.ReadZSputnikSeed_deprecated(seed)) {
            //Update to new format, erase old
            seedMaster = seed;
            hashSeed = Hash(seed.begin(), seed.end());
            if (pwalletMain->AddDeterministicSeed(seed)) {
                if (walletdb.EraseZSputnikSeed_deprecated()) {
                    LogPrintf("%s: Updated zSputnikseed databasing\n", __func__);
                    fFirstRun = false;
                } else {
                    LogPrintf("%s: failed to remove old zSputnik seed\n", __func__);
                }
            }
        }
    }
    ...
    if (fFirstRun) {
        ...
        seedMaster = seed;
        LogPrintf("%s: first run of zSputnik wallet detected, new seed generated. Seedhash=%s\n", __func__, Hash(seed.begin(), seed.end()).GetHex());
...
bool CzSputnikWallet::SetMasterSeed(const uint256& seedMaster, bool fResetCount)
{
    ...
    if (fResetCount)
        walletdb.WriteZSputnikCount(nCountLastUsed);
    else if (!walletdb.ReadZSputnikCount(nCountLastUsed))
        nCountLastUsed = 0;
...
void CzSputnikWallet::Lock()
{
    seedMaster = 0;
}

void CzSputnikWallet::AddToMintPool(const std::pair<uint256, uint32_t>& pMint, bool fVerbose)
{
    mintPool.Add(pMint, fVerbose);
}

//Add the next 20 mints to the mint pool
void CzSputnikWallet::GenerateMintPool(uint32_t nCountStart, uint32_t nCountEnd)
{
    ...
    for (uint32_t i = n; i < nStop; ++i) {
        ...
        CKey key;
        SeedToZSputnik(seedZerocoin, bnValue, bnSerial, bnRandomness, key);
...
bool CzSputnikWallet::LoadMintPoolFromDB()
{
...
void CzSputnikWallet::RemoveMintsFromPool(const std::vector<uint256>& vPubcoinHashes)
{
    for (const uint256& hash : vPubcoinHashes)
        mintPool.Remove(hash);
}

void CzSputnikWallet::GetState(int& nCount, int& nLastGenerated)
{
...
void CzSputnikWallet::SyncWithChain(bool fGenerateMintPool)
{
    ...
    while (found) {
        ...
        for (pair<uint256, uint32_t> pMint : listMints) {
            ...
            if (pwalletMain->zSputnikTracker->HasPubcoinHash(pMint.first)) {
...
bool CzSputnikWallet::SetMintSeen(const CBigNum& bnValue, const int& nHeight, const uint256& txid, const CoinDenomination& denom)
{
    ...
    CBigNum bnRandomness;
    CKey key;
    SeedToZSputnik(seedZerocoin, bnValueGen, bnSerial, bnRandomness, key);
    ...
    // Add to zSputnikTracker which also adds to database
    pwalletMain->zSputnikTracker->Add(dMint, true);
...
void CzSputnikWallet::SeedToZSputnik(const uint512& seedZerocoin, CBigNum& bnValue, CBigNum& bnSerial, CBigNum& bnRandomness, CKey& key)
{
...
uint512 CzSputnikWallet::GetZerocoinSeed(uint32_t n)
{
...
void CzSputnikWallet::UpdateCount()
{
    nCountLastUsed++;
    CWalletDB walletdb(strWalletFile);
    walletdb.WriteZSputnikCount(nCountLastUsed);
...
void CzSputnikWallet::GenerateDeterministicZSputnik(CoinDenomination denom, PrivateCoin& coin, CDeterministicMint& dMint, bool fGenerateOnly)
{
...
void CzSputnikWallet::GenerateMint(const uint32_t& nCount, const CoinDenomination denom, PrivateCoin& coin, CDeterministicMint& dMint)
{
...
bool CzSputnikWallet::RegenerateMint(const CDeterministicMint& dMint, CZerocoinMint& mint)
{
