...
BOOST_AUTO_TEST_CASE(zerocoin_spend_test)
{
    ...
    cWallet.zSputnikTracker = unique_ptr<CzSputnikTracker>(new CzSputnikTracker(cWallet.strWalletFile));
    ...
    BOOST_CHECK_MESSAGE(receipt.GetStatus() == ZSputnik_TRX_FUNDS_PROBLEMS, "Failed Invalid Amount Check");
    ...
    BOOST_CHECK_MESSAGE(receipt2.GetStatus() == ZSputnik_TRX_FUNDS_PROBLEMS, "Failed Invalid Amount Check");
